...
void MyList2<T>::insert(const T&elem, iterator whereIt) { 	
	...
	if(whereIt.ptr == begin()){		//caso especial inicio lista
		push_front(elem);
	} 
	else if(whereIt.ptr == end()){    //caso especial no final ou lista fazia
		push_back(elem);
	}
	else{
		Node<T>* newNode = new Node<T>(elem);
		newNode->next = whereIt.ptr;      
        newNode->prev = whereIt.ptr->prev; 
        whereIt.ptr->prev->next = newNode; 
        whereIt.ptr->prev = newNode;      
        dataSize++;
	}
}
...
template<class T>
int MyList2<T>::size(Node<T>* node) const {
    if (node == nullptr) {
        return 0; 
    }
    return 1 + size(node->prev);  
}
