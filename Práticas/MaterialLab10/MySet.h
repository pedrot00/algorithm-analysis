...
template  <class T>
class MySet {			//faltam: destrutor, construtor por copia, opedador = 
public:
...
private:
	...
	Node<T>* copyNode(const Node<T>*root, Node<T>*parent);
...
// --------------------ETAPA 1----------------//
template<class T>
void MySet<T>::deleteNode(Node<T>*ptr){		//aux destrutor
	if(!ptr) return;
	deleteNode(ptr->left);
...
template<class T>
MySet<T>:: ~MySet(){			//destrutor
	deleteNode(root);
	size_ = 0;
}

template <class T>
MySet<T>:: MySet(const MySet<T>&other){		//construtor por copia
	root = nullptr; size_ = 0;		//pra evitar problema caso o copyNode falhe na verificacao
...
template <class T>
Node<T>* MySet<T>::copyNode(const Node<T>*ptr, Node<T>*parent){	//aux construtor por copia
	if(!ptr) return nullptr;

	Node<T>* current = new Node<T>(ptr->elem);
	current->parent = parent;
	
	if(ptr->left) current->left = copyNode(ptr->left, current);
	if(ptr->right) current->right = copyNode(ptr->right, current);
